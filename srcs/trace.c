...
/*   Updated: 2018/08/16 12:39:40 by wgourley         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	touch(t_value angle, t_player p, t_map *m)
{
	...
	c[1] = 255 * (cang / 3);
	c[2] = 255 * ((3 - cang) / 3);
	c[0] *= ((VIEW_MAX - pp[2]) / VIEW_MAX);
	c[1] *= ((VIEW_MAX - pp[2]) / VIEW_MAX);
	c[2] *= ((VIEW_MAX - pp[2]) / VIEW_MAX);
	pp[0] = (angle / FOV) * WINDOW_W;
	last = clone_value_v(pp, 2);
	pp[1] = WINDOW_C_H + pow(((VIEW_MAX - pp[2]) / VIEW_MAX) * WINDOW_C_H, 0.9);
	last[1] = WINDOW_C_H - pow(((VIEW_MAX - pp[2]) / VIEW_MAX) * WINDOW_C_H, 0.9);
